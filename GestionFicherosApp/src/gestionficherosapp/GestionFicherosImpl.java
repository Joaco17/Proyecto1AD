	...
	public void creaCarpeta(String arg0) throws GestionFicherosException {
		...
		try{
			if(file.exists()==false && carpetaDeTrabajo.canWrite()==true){
					...
					//Si baja la ejecucion y se entra  aquí, se muestra una ventanita informativa y se muestra un mensaje por consola
			...
			if(file.exists()==true || file.canRead()==true){
				...
				try{
					if(file.isFile()==true){
						...
						strBuilder.append(formatea.format(tamañoBytes)+" bytes");
						strBuilder.append("\n");
					}
